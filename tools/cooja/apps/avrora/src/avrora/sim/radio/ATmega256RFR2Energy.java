...
/**
 * Constants for Atmel ATmega256RFR2 radio energy consumption
 * 
 * @author David Kopf
 */
public abstract class ATmega256RFR2Energy {  
    ...
    public static final double[] modeAmpere = {                                             
        ...
        0.000400,  //TRX_OFF
        0.0057,    //P_ON, PLL_ON
        0.0125,    //Software CCA state, to signal the event to Cooja
 ...
 //     0.0100,    //           (1.0 dBm)
    ...
    public static String[] allModeNames() {
        String[] modeName = new String[16+7];

        System.arraycopy(ATmega256RFR2Energy.modeName, 0, modeName, 0, 7);
        for (int i = 0; i < 16; i++) {
            modeName[i + 7] = ATmega256RFR2Energy.modeName[7] + StringUtil.leftJustify(i+":  ", 2);
        }
        return modeName;
    }
}
